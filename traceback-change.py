...
#==============change============'return None'
            ...
            if module_name not in sys.modules:
                if len(wrong_name_list) > 1 and _closed_name := _calculate_closed_name(module_name, list(sys.stdlib_module_names)):
                    return _closed_name
                ...
                if hasattr(sys.modules[module_name], '__path__') and len(wrong_name_list)>1:
                    ...
                    for i in wrong_name_list[1:]:
                        ...
                        if module_name not in sys.modules:
                            wrong_name = i
                            d = find_all_packages.scan_dir(sys.modules[original_module_name].__path__[0])
                            break
                        else:
